...
class VectorIndexing(BaseIndexing):
    ...
    def add_to_vectorstore(self, docs: list[Document]):
        #lmm checks 
        # Step 1: Validate Document Text Length
        for idx, doc in enumerate(docs):
            if len(doc.text) > 65536:
                print(f"Document text at index {idx} exceeds 65536 characters (current length: {len(doc.text)}). Truncating.")
                doc.text = doc.text[:55536]  # Truncate text to meet the limit
        ...
        # Step 3: Validate Metadata Total Size
        for idx, doc in enumerate(docs):
            metadata = doc.metadata
            serialized_metadata = json.dumps(metadata)
            metadata_size = len(serialized_metadata.encode('utf-8'))
            if metadata_size > 65536:
                print(f"Total metadata size for document index {idx} exceeds 65536 bytes (current size: {metadata_size}). Adjusting metadata.")
                # Prioritize essential fields
                essential_fields = {}
                current_size = 0
                for key, value in metadata.items():
                    # Serialize the single key-value pair to measure its size
                    field_serialized = json.dumps({key: value})
                    field_size = len(field_serialized.encode('utf-8'))
                    if current_size + field_size <= 65536:
                        essential_fields[key] = value
                        current_size += field_size
                    else:
                        print(f"Skipping metadata field '{key}' to keep within size limit.")
                # Assign the adjusted metadata back to the document
                doc.metadata = essential_fields
                print(f"Adjusted metadata size for document index {idx} is {current_size} bytes.")
            else:
                print(f"Metadata size for document index {idx} is within the limit: {metadata_size} bytes.")


        # kotaemon code  --------------------------------------------------
        # in case we want to skip embedding
        if self.vector_store:
            # Ensure embeddings are not recalculated if they are already computed
            ...
            for idx, embedding in enumerate(embeddings):
                if embedding is None:
                    print(f"Failed to generate embedding for document index {idx}. Skipping.")
                else:
                    valid_docs.append(docs[idx])
                    valid_embeddings.append(embedding)

            # Update docs and embeddings
            docs = valid_docs
