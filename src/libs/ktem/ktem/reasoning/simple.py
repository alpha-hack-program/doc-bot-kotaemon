...
class AddQueryContextPipeline(BaseComponent):
    ...
    def run(self, question: str, history: list) -> Document:
        messages = [
            SystemMessage(
            content="A continuación se muestra un historial de la conversación hasta ahora, y una nueva "
                "pregunta hecha por el usuario que debe ser respondida mediante la búsqueda "
                "en una base de conocimiento.\nTienes acceso a un índice de búsqueda "
                "con cientos de documentos.\nGenera una consulta de búsqueda basada en la "
                "conversación y la nueva pregunta.\nNo incluyas nombres de archivos citados "
                "como info.txt o doc.pdf en los términos de la consulta de búsqueda.\nNo incluyas caracteres especiales como '+'.\nSi la pregunta no está en inglés, reescribe la consulta en "
                "el idioma usado en la pregunta.\nSi la pregunta contiene suficiente "
                "información, devuelve solo el número 1.\nSi no es necesario hacer "
                "la búsqueda, devuelve solo el número 0."
            ),
            HumanMessage(content="¿Cómo le fue al cripto el año pasado?"),
            AIMessage(
                content="Resume la dinámica del mercado de criptomonedas del año pasado"
            ),
            HumanMessage(content="¿Cuáles son mis planes de salud?"),
            AIMessage(content="Mostrar los planes de salud disponibles"),
        ]
        for human, ai in history[-self.n_last_interactions :]:
            messages.append(HumanMessage(content=human))
            messages.append(AIMessage(content=ai))

        messages.append(HumanMessage(content=f"Genera una consulta de búsqueda para: {question}"))
...
class FullQAPipeline(BaseReasoning):
        ...
        if not with_citation and not without_citation:
            ...
            if has_llm_score and max_llm_rerank_score < CONTEXT_RELEVANT_WARNING_SCORE:
                yield Document(
                    channel="info",
                    content=(
                        "<h5>¡ADVERTENCIA! El puntaje de relevancia del contexto es bajo. "
                        "Verifica dos veces la respuesta del modelo para asegurarte de su exactitud.</h5>"
                    ),
    ...
    def get_info(cls) -> dict:
        return {
            ...
            "name": "Simple QA",
            "description": (
                "Pipeline simple de preguntas y respuestas basado en RAG. Este pipeline puede "
                "realizar tanto búsquedas por palabras clave como búsquedas por similitud para recuperar el "
                "contexto. Después, incluye ese contexto para generar la respuesta."
            ),
        }
